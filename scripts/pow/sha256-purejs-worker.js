...
// SHA-256 核心来自 @aws-crypto/sha256-js 5.2.0 的 build/module 编译产物。
const BLOCK_SIZE = 64;
...
self.addEventListener("message", ({ data: eventData }) => {
  ...
  function runBatch() {
    ...
    for (let batchIndex = 0; batchIndex < 1024; batchIndex += 1) {
      const hashArray = calculateSHA256(data + nonce);
